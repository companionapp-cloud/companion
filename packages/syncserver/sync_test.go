...
func newServerAPI(t *testing.T) (*httptest.Server, *Server) {
	...
	if dialect == "postgres" {
		...
		if _, err := db.Exec(`TRUNCATE users, sessions, refresh_tokens, user_seq, notes, tasks, areas, projects, project_members, user_secrets, canvases, canvas_nodes, canvas_edges, note_ink, notebooks, notebook_pages, notebook_page_ink;`); err != nil {
