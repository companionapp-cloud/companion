...
Select more than one note, task or canvas in a list and choose **Export…** in the bar above the selection. **⌘-click** (Ctrl-click on Windows and Linux) adds items one by one, and **⇧-click** takes a range. **File › Export** does the same while a selection is active.
...
- **PNG**: one image. A note or task is a single tall sheet, and a canvas is its own size.
...
The repository and its sign-in are synced to your other devices end-to-end encrypted, so the Companion server can't read them. (If your account isn't end-to-end encrypted, the sign-in stays on the device you typed it into, and a computer taking the sync over asks for it.) A computer that takes the sync over works out where things stand from the repository itself: files left behind by things you've since renamed, moved or deleted are tidied away, anything newer in Companion is written out, and edits made in the repository still come in. Git sync runs in the desktop app, while it's open or in the menu bar.
...
## Exports on your other devices

Each filesystem export and Git sync runs on one computer: the one you set it up on, or the last one to take it over. Every device you're signed in on lists it, under **Settings › Export** or **Settings › Sync**, with the computer that runs it, how its last run went, and when that computer was last online.

From any device, the web app and the phone apps included, you can:

- **Pause** it, or **Resume** it.
- **Remove** it. The files it already wrote stay where they are.
- **Take over…** (desktop app only), so this computer runs it from now on. A filesystem export asks which folder to use on this computer; if a cloud drive keeps the same folder on both, choose that one. A Git sync asks you to sign in if this computer doesn't have the sign-in yet.

The computer that ran it picks up the change the next time it connects to Companion, and until then the export's row says so. Setting up a second export to a folder or repository another computer already uses offers to take that one over instead.
...
- Each Markdown file becomes a note, or a task if its folder or `kind` says so. A file with no frontmatter is a note named after the file.
- `[[Links]]` between the files are kept, and an `Areas/…` layout files things into areas and projects, creating them as needed.
- Images and files the notes embed come along, whether written `![[photo.png]]` or plain `![](images/photo.png)`, wherever in the folder they sit. Files no note uses are left behind.
