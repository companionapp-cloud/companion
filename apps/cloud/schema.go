...
CREATE TABLE IF NOT EXISTS subscriptions (
  ...
  -- Renewal was turned off only because the account is scheduled for deletion (deletion.go),
  -- so a sign-in that takes the deletion back turns it on again. A cancellation the user
  -- scheduled themselves leaves this at 0 and stays theirs.
  paused_for_deletion    BIGINT NOT NULL DEFAULT 0,
  created_at             TEXT NOT NULL,
  updated_at             TEXT NOT NULL
);
...
func applyCloudSchema(db *sql.DB, dialect string) error {
	...
	// self-serve cancellation, and account deletion).
	alters := []string{
		...
		`ALTER TABLE subscriptions ADD COLUMN paused_for_deletion BIGINT NOT NULL DEFAULT 0`,
	}
